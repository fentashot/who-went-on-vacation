...
const leetifyRequestSchema = z.object({
  steamId: z
    .string()
    .min(1, "Steam ID is required")
    .regex(/^\d{17}$/, "Invalid Steam64 ID. Must be a 17-digit number"),
});
...
const app = new Hono()
  .basePath("/api/v2/")
  // @POST /api/v2/steam
  .post("/steam", zValidator("json", steamRequestSchema), async (c) => {
    try {
      if (!STEAM_API_KEY || STEAM_API_KEY === "your_steam_api_key_here") {
        return c.json(
          {
            error:
              "Steam API key not configured. Please add STEAM_API_KEY to your .env file.",
          },
          500
        );
      }

      const { profileUrl } = c.req.valid("json");
      ...
      const allFriends: FriendProfile[] = allPlayers.map((player) => {
        const banInfo = banStatuses.find(
          (ban) => ban.SteamId === player.steamid
        );
        return {
          ...player,
  ...
  // @POST /api/v2/leetify
  .post("/leetify", zValidator("json", leetifyRequestSchema), async (c) => {
    try {
      ...
      if (!profile) {
        const response: LeetifyApiResponse = {
          stats: null,
